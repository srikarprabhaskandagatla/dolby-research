...
harmony_features = pd.read_csv(GENES_TSV, sep="\t").iloc[7:9]
HARMONY_FEATURES = []
...
SYSTEM_PROMPT = (
    "You are an expert musicologist annotating harmonic characteristics of songs. "
...
FEATURE_DEFINITIONS = {
    "Minor / Major Key Tonality": (
        'The harmonic tonality of the song on a scale from minor to major. '
        'Lowest scores (0) indicate minor tonality throughout the song. '
        'Highest scores (5) indicate major tonality throughout the song. '
        'Middle scores (2-3) indicate ambiguity in tonality. '
        'Judge by the tonic chord quality, not mood or tempo.'
    ),
    "Harmonic Sophistication": (
        'The complexity level of a track\'s harmony based on chord analysis. '
        '0=only I, IV, V; '
        '1=adds diatonic chords (ii, iii, vi); '
        '2=includes secondary dominants (e.g. V/V); '
        '3=other non-diatonic chords; '
        '4=frequent chromaticism; '
        '5=extreme chromaticism. '
    ),
}

HARMONY_FEATURE_INDICES = {
    'Minor/Major Key Tonality': 7,
    'Harmonic Sophistication': 8,
}
...
def build_user_prompt(artist: str, title: str) -> str:
    ...
    return (
        ...
        f'Rate each harmonic feature from 0 (lowest) to 5 (highest):\n'
        f'{feature_block}\n\n'
        f'Return a JSON object with exactly these keys: {HARMONY_FEATURES}'
    )
...
def evaluate_annotations(df: pd.DataFrame) -> pd.DataFrame:
    ...
    rows = []
    for feat, idx in HARMONY_FEATURE_INDICES.items():
...
csv_path = os.path.join(OUTPUT_DIR, f"harmony_features_part{args.part}.csv")
